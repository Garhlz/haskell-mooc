...
data State = Start | Error | Finished | AfterEgg | AfterFlour | AfterSugar | AfterFlourAndSugar | AfterMixing
  deriving (Eq, Show)
...
step AfterFlourAndSugar Mix = AfterMixing
step AfterMixing Bake = Finished
step Finished _ = Finished
step _ _ = Error -- 其他状态转移途径都是错误的
...
reverseNonEmpty (x :| xs) = go xs (x :| [])
  where
    go [] acc = acc -- 普通列表可以为空
    go (x : xs) (y :| ys) = go xs (x :| (y : ys)) -- 第一个参数是普通列表，第二个是非空列表
    -- 因为NonEmpty只能匹配为 x:|xs, 所以必须用这个有意义的形式表示它
...
passwordAllowed s (ContainsSome set) = not $ null (intersect s set)
passwordAllowed s (DoesNotContain set) = null (intersect s set)
...
data Arithmetic = Anum Integer | Aope String Arithmetic Arithmetic
  deriving (Show)
...
evaluate (Anum x) = x
evaluate (Aope "+" a b) = (+) (evaluate a) (evaluate b)
evaluate (Aope "*" a b) = (*) (evaluate a) (evaluate b)

render :: Arithmetic -> String
render (Anum x) = show x
render (Aope s a b) = "(" ++ render a ++ s ++ render b ++ ")"