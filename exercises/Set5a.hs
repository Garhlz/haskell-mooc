...
totalPrice s = case s of
  (MkShoppingEntry _ price count) -> price * fromIntegral count
...
buyOneMore s = case s of
  (MkShoppingEntry a b count) -> MkShoppingEntry a b (count + 1)
...
study s = case s of
  (NthYear 7) -> Graduated
  Freshman -> NthYear 1
  (NthYear x) -> if x >= 1 && x <= 6 then NthYear (x + 1) else NthYear x
  Graduated -> Graduated
...
fromBin b = go b 0 1
  where
    go End acc _ = acc
    go (I b) acc cur = go b (acc + cur) (cur * 2)
    go (O b) acc cur = go b acc (cur * 2)
...
toBin x
  ...
  | odd x = I (toBin (div x 2))
  | otherwise = O (toBin (div x 2))