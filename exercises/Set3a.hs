...
powers :: Int -> Int -> [Int]
powers k limit = takeWhile (<= limit) $ map (k ^) [0 ..]

-- powers k limit = takeWhile (<= limit) (iterate (* k) 1)
...
joinToLength t xss = [a ++ b | a <- xss, b <- xss, length a + length b == t]
...
multiCompose [] a = a
multiCompose (f : fx) a = f $ multiCompose fx a

-- multiCompose' :: [a -> a] -> a -> a
-- multiCompose' fx a = foldr ($) a fx

-- multiCompose'' fx a = foldr (.) id fx a
...
interpreter commands = go commands 0 0
  where
    ...
    go (s : ss) x y
      ...
      | s == "right" = go ss (x + 1) y
      | s == "left" = go ss (x - 1) y
      | s == "printX" = show x : go ss x y
      | s == "printY" = show y : go ss x y
      | otherwise = go ss x y
